...
class ActionTopRateAddress(Action):
    ...
    def run(
        ...
        if (address_food) and (food_name):
            eatery_spots = action_service.get_spots_by_foodname_address(food_name=food_name, address_food=address_food, use_sort=True, use_limit=n_top_spots)
            ...
            if eatery_spots:
                ...
                for i, spot in enumerate(eatery_spots):
                    text += f"{i+1}. {spot['name']} ở {spot['address']}\n"
                    text += f"----> link: {spot['link']}\n"
            dispatcher.utter_message(text=text)
            return []
        
        if (not address_food) and (food_name):
            eatery_spots = action_service.get_spots_by_foodname_address(food_name=food_name, use_sort=True, use_limit=n_top_spots)
        ...
        if (address_food) and (not food_name):
            eatery_spots = action_service.get_spots_by_foodname_address(address_food=address_food, use_limit=n_top_spots)
        ...
        dispatcher.utter_message(text="Xin lỗi nhưng Bot không hiểu bạn muốn tìm thông tin về đồ ăn hay quán ăn nào?")
        return []
...
class ActionFoodNameWithAddress(Action):
    ...
    def run(
        ...
        if food_name and address_food:
            list = action_service.get_spots_by_foodname_address(food_name, address_food)
            if list is None or len(list) == 0:
...
class ActionFoodTime(Action):
    ...
    def run(
        ...
        for e in entities:
            if e["entity"] == "food_name":
                food_name = e["value"]
            elif e["entity"] in ("start_time", "end_time"):
                if number==0:
                    start_time = e["value"]
                    number += 1
                else:
                    end_time = e["value"]
            elif e["entity"] == "time_type":
                time_type = e["value"]
            elif e["entity"] == "address_food":
                address_food = e["value"]
            
        if food_name and start_time and end_time and address_food:
            tBotp_objs = action_service.get_food_name_with_time_address(food_name, start_time, end_time, address_food)
            if not tBotp_objs:
            ...
            if not tBotp_objs:
                ...
                dispatcher.utter_message(text="Bot xin gợi ý các quán có món ăn " + food_name + " tại " + address_food + " vào thời điểm hiện tại là: \n" + "\n".join([f"* Tên quán: {obj['name']}, Địa chỉ: {obj['address']}, Link mua hàng: {obj['link']}" for obj in tBotp_objs]))
        elif food_name and address_food and time_type and start_time:
            tBotp_objs = action_service.get_food_name_with_type_time_address1(food_name, start_time, time_type, address_food)
            if not tBotp_objs:
            ...
            tBotp_objs = action_service.get_time_address(start_time, end_time, address_food)
            if not tBotp_objs:

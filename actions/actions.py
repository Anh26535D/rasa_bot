...
class ActionFoodTime(Action):
    ...
    def run(
        ...
        if food_name and start_time and end_time and address_food:
            tBotp_objs = action_service.get_food_name_with_time_address(
            ...
            tBotp_objs = action_service.get_food_name_with_now_address(
                food_name, address_food)
            if not tBotp_objs:
                dispatcher.utter_message(text="Bot xin lỗi, hiện tại không tìm thấy dữ liệu về món ăn " +
            ...
            tBotp_objs = action_service.get_food_name_with_type_time_address1(
            ...
            if not tBotp_objs:
                ...
                dispatcher.utter_message(text="Bot xin gợi ý các quán có món ăn " + food_name + " vào thời điểm hiện tại là: \n" + "\n".join(
                    [f"* Tên quán: {obj['name']}, Địa chỉ: {obj['address']}, Link mua hàng: {obj['link']}" for obj in tBotp_objs]))
            ...
            tBotp_objs = action_service.get_time_address(
            ...
            if not tBotp_objs:
                ...
                dispatcher.utter_message(text="Bot xin gợi ý các món tại " + address_food + " vào thời điểm hiện tại là: \n" + "\n".join(
                    [f"* Tên quán: {food['Tên quán']}, Địa chỉ: {food['Địa chỉ']}, Link mua hàng: {food['Url']}" for food in tBotp_objs]))
        else:
            dispatcher.utter_message(
                text="Xin lỗi bạn, Bot không hiểu bạn muốn tìm món ăn gì hay ở đâu?")
